...
<div style="text-align: center; padding: 20px;">
    <form method="post" action="{{url('add_fill.php')}}" style="background-color: #fff; padding: 20px; border-radius: 8px; box-shadow: 0 0 10px rgba(0, 0, 0, 0.1);">
        <h2>Add Filiere</h2>
...
@foreach($filieres as $announcement)
  <div class="announcement-container">
    <div>
      <h2>{{ $announcement->filiere }}</h2>
    </div>
    
    <div>
      <button class="edit-button" onclick="openModal('{{ $announcement->id }}', '{{ $announcement->filiere }}')">Edit</button>
      <form action="{{url('delete_fill.php')}}" method="POST" class="delete-form">
        @csrf
  ...
  <div id="editModal" class="modal">
    <div class="modal-content">
      ...
      <form id="editForm" method="post" action="{{ url('edit_fill.php') }}">
        @csrf
        <input type="hidden" id="editId" name="id" value="">

        <label for="title">Filiere:</label>
        <input type="text" id="editFill" name="title" required>
