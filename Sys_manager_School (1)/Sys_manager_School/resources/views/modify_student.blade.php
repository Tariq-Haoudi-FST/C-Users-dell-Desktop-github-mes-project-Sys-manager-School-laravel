...
<div style="text-align: center; padding: 20px;">
    <form method="post" action="{{url('add_std.php')}}" style="background-color: #fff; padding: 20px; border-radius: 8px; box-shadow: 0 0 10px rgba(0, 0, 0, 0.1);">
        <h2>Add Student</h2>
    ...
    <label for="cm" style="display: block; text-align:left; margin-bottom: 8px;">Code Massar:</label>
        <input type="text" id="cm" name="cm" required style="width: 100%; padding: 8px; margin-bottom: 16px; box-sizing: border-box; border: 1px solid #ccc; border-radius: 4px;">
    ...
    <label for="groupe" style="display: block; text-align:left; margin-bottom: 8px;">Groupe:</label>
        <input type="text" id="groupe" name="groupe"  style="width: 100%; padding: 8px; margin-bottom: 16px; box-sizing: border-box; border: 1px solid #ccc; border-radius: 4px;">
...
@foreach($student as $announcement)
  <div class="announcement-container">
    ...
    <p>Code Massar: {{ $announcement->code_massar }}</p>
    <p>Filiere: {{ $announcement->filiere }}</p>
    <p>Groupe: {{ $announcement->Groupe }}</p>
    </div>
    

    <div>
      <button class="edit-button" onclick="openModal('{{ $announcement->id }}', '{{ $announcement->dept }}')">Edit</button>
      <form action="{{url('delete_std.php')}}" method="POST" class="delete-form">
        @csrf
  ...
  <div id="editModal" class="modal">
    <div class="modal-content">
      ...
      <form id="editForm" method="post" action="{{ url('edit_std.php') }}">
        @csrf
        ...
        <label for="title">Code Massar:</label>
        <input type="text" id="editMassar" name="massar" required>

        <label for="title">Filiere:</label>
        <input type="text" id="editfill" name="fill" required>
        <label for="title">Groupe:</label>
        <input type="text" id="editGroupe" name="group" required>
...
<script>
  ...
  function openModal(id,nom_prenom,adresse,date_de_naissance,cin,cm, filiere, dept) {
    ...
    document.getElementById('editMassar').value = cm; 
    document.getElementById('editfill').value = filiere; 
    document.getElementById('editGroupe').value = dept; 
